...
class StubOnlineFileRequest : public AsyncRequest {
public:
    StubOnlineFileRequest(StubOnlineFileSource& fileSource_)
        : fileSource(fileSource_) {
    }

    ~StubOnlineFileRequest() override {
        fileSource.remove(this);
    }
...
StubOnlineFileSource::StubOnlineFileSource(ResponseType type_)
        : type(type_) {
    if (type == ResponseType::Synchronous) {
        return;
    }
    ...
    timer.start(1ms, 1ms, [this] {
        // Explicit copy to avoid iterator invalidation if ~StubOnlineFileRequest gets called within the loop.
        auto pending_ = pending;
        for (auto& pair : pending_) {
            optional<Response> res = std::get<1>(pair.second)(std::get<0>(pair.second));
            if (res) {
...
StubOnlineFileSource::~StubOnlineFileSource() = default;

std::unique_ptr<AsyncRequest> StubOnlineFileSource::request(const Resource& resource, Callback callback) {
    auto req = std::make_unique<StubOnlineFileRequest>(*this);
    if (type == ResponseType::Synchronous) {
        optional<Response> res = response(resource);
        if (res) {
            callback(*res);
...
void StubOnlineFileSource::remove(AsyncRequest* req) {
    auto it = pending.find(req);
    if (it != pending.end()) {
        pending.erase(it);
    }
...
optional<Response> StubOnlineFileSource::defaultResponse(const Resource& resource) {
    switch (resource.kind) {
    case Resource::Kind::Style:
        if (!styleResponse) throw std::runtime_error("unexpected style request");
        return styleResponse(resource);
